...
def age_from(age_user, user_id):
    if age_user is None or len(age_user.split('.')) < 3:
        send_msg({'user_id': user_id, 'message': 'Возраст не указан. Укажите Нижний предел',
...
def age_to(age_user, user_id):
    if age_user is None or len(age_user.split('.')) < 3:
        send_msg({'user_id': user_id, 'message': 'Возраст не указан. Укажите Верхний предел',
                  'random_id': randrange(10 ** 7)})
        return int(wait_msg().text)
    res = datetime.now().year - int(age_user.split('.')[2])
    return res + 5
...
def check_city(user_field, user_id):
    if user_field is None or user_field == '':
        send_msg({'user_id': user_id, 'message': 'Город не заполнен. Укажите в ответном сообщении',
                  'random_id': randrange(10 ** 7)})
...
def check_country(user_field, user_id):
    if user_field is None or user_field == '':
        send_msg({'user_id': user_id, 'message': 'Страна не заполнена. Укажите в ответном сообщении',
                  'random_id': randrange(10 ** 7)})
        return wait_msg().text
    return user_field['id']
...
def search_user(user_fields):
    ...
    params = {'q': '', 'sort': 0, 'online': 1, 'has_photo': 1, 'count': 1000,
              ...
              'country': check_country(user_fields.get('country'), user_fields['id'])}  # указать пол
...
def bot_answer(event):
    ...
    if request.lower() == "привет" or request.lower() == 'hi':
        ...
        send_msg(
            {'user_id': event.user_id, 'message': f"пока {user_fields['first_name']}", 'random_id': randrange(10 ** 7)})
    else:
